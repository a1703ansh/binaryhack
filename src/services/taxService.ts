import { TaxProfile } from '../types/index.ts';
...
export function computeTaxStatus(
  ...
  const estimatedAnnualIncome = monthlyIncomeRunRate * 12; // ~ ₹3,42,000
  // Target tax reserve ~ 1.5% to 3% effective provision for gig regime demonstration
  const annualTargetProvision = Math.round(estimatedAnnualIncome * 0.015); // ~ ₹5,130
  const quarterlyTarget = Math.round(annualTargetProvision / 4) * 4; // ~ ₹5,000
