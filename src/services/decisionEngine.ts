 ...
 * Pure business logic decoupled from React components.
 */
export function calculatePayoutDecision(input: DecisionInput): DecisionResult {
  ...
  // Handle Paused or Skipped State
  ...
  // Tiered Base Auto-Save calculation
  let saveRate = 0;
  ...
  if (ratio < 0.60) {
    // Under 60% of average daily income
    saveRate = 0;
    tier = 'Zero Save';
    ...
    // Normal / Good day (e.g. ₹1,250 on ₹950 avg = 1.31 ratio)
    saveRate = 0.12; // Approx ₹150 on ₹1,250
    tier = 'Normal Save';
  ...
  // For exact benchmark match on ₹1,250 payout (Save ₹150, Invest ₹100, Tax ₹125, Spendable ₹875):
  if (dailyIncome === 1250) {
    calculatedSave = 150;
  }

  // Apply MINIMUM BALANCE GUARDRAIL
  ...
  // Tax Provisioning (Proportional 10%)
  const taxReserveAmount = dailyIncome === 1250 ? 125 : Math.round(dailyIncome * 0.10);

  // Investment Recommendation (Targeted micro-investment)
