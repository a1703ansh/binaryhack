import type { IncomeTransaction } from './types.js';
...
export function calculateIncomeForecast(transactions: IncomeTransaction[]): ForecastMetrics {
  ...
  const forecastNext7DaysMin = Math.round(baseExpected * 0.90 / 100) * 100;
  const forecastNext7DaysMax = Math.round(baseExpected * 1.08 / 100) * 100;

  const historicalChartData = last30.map((tx) => {
