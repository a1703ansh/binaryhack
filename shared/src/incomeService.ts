import type { IncomeSource, IncomeTransaction, Platform } from './types.js';
...
/**
 * Validate & normalize imported CSV transactions (Date, Amount, Platform, Description).
 * Amounts must parse to a positive number; unknown platforms map to 'Other'.
 */
export function sanitizeCSVTransactions(rows: Partial<IncomeTransaction>[], validPlatforms: readonly string[]): IncomeTransaction[] {
  ...
  return rows
    ...
    .map((r, idx) => ({
      ...
      source: (validPlatforms.includes(r.source as string) ? r.source : 'Other') as Platform,
      amount: r.amount as number,
      date: r.date || today,
      description: r.description || 'Imported statement record',
